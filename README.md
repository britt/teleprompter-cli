...
A modern CLI for managing LLM prompts with Cloudflare Workers. Features an interactive terminal UI built with [Ink](https://github.com/vadimdemedes/ink), full scriptability with JSON output, and support for prompts that carry dotprompt metadata such as model information.

Teleprompter CLI is a command-line interface for interacting with [Teleprompter](https://github.com/britt/teleprompter/). Teleprompter manages prompts for Large Language Model (LLM) applications at runtime on Cloudflare and Cloudflare Workers. It provides versioning, metadata tracking, runtime editing and updating of prompts, and visibility into prompt metadata such as model selection when that metadata is present.
...
- **View details**: Full prompt information with version history and a `Metadata` section for dotprompt metadata such as model, description, config, tools, and input or output schema details
...
- **Test prompts**: Prompt and template views show the executable template body and handle YAML frontmatter metadata separately
- **Keyboard controls**:
  - `↑/↓` - Navigate lists
...
`tp put` accepts plain prompt text, full dotprompt source passed as the text argument, or dotprompt source piped through standard input. When YAML frontmatter is present, the CLI treats it as prompt metadata rather than as template variables, and prompt and template displays show only the template body.

Use a dotprompt format like this when a prompt needs metadata:
...
Plain prompts without frontmatter still work. Prompts without model metadata simply do not show a model value in places such as the prompt list.

`tp export` writes JSON files that include `id`, `namespace`, `prompt`, and `metadata`. `tp import` preserves that metadata during round tripping by importing the prompt source and keeping metadata available in exported files.

If prompt metadata or YAML frontmatter is malformed, `tp put` and `tp import` return validation focused error messages instead of only a generic HTTP 400 failure. JSON mode returns the same validation details in structured error output.
